...
class TSPGUI:
    def __init__(self, root):
        ...
        tk.Label(root, text="Travelling Salesman Problem",font=('Times New Roman',18), bg='lightblue').grid(row=0, column=2, sticky='w')
        ...
        self.forts_entry = tk.Entry(root, width=50)
        self.forts_entry.grid(row=1, column=2, columnspan=3)
        # Input for distance matrix
        tk.Label(root, text="Distance matrix (comma-separated values, rows separated by semicolons):",font=('arial',15), bg='lightblue').grid(row=2, column=1, sticky='w')
        self.matrix_entry = tk.Entry(root, width=50)
        self.matrix_entry.grid(row=2, column=2, columnspan=3)
        # Solve button
        self.solve_button = tk.Button(root, text="Solve TSP", command=self.solve_tsp, font=('arial',15), bg='lightblue')
        self.solve_button.grid(row=3, column=2, columnspan=2, )
        # Placeholder for result label
        self.result_label = tk.Label(root, text="",font=('arial',15), bg='lightblue')
    ...
    def create_new_window(self):
        ...
        new_root = tk.Tk()
        new_app = TSPGUI(new_root)  # Create a new instance of the app
